(ns liquid-robotics.main
  "Kotodama WASM entrypoint for the Liquid_robotics clean-room actor (L4) — Clojure port.
  ...
  facts (`liquid_robotics.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "liquid_robotics")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"liquid_robotics.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "liquid_robotics-compat" :tier tier :entities entities} 200])
...
(defn start! [] :liquid_robotics-compat/ready)
